...
GstElement * CameraDevice::CreateVideoPipeline(const std::string & device, int width, int height, int framerate,
                                               CameraError & error)
{
    GstElement *pipeline, *source, *capsfilter, *videoconvert, *videoscale, *x264enc, *rtph264pay;
    ...
    if (!source || !capsfilter || !videoconvert || !videoscale || !x264enc || !rtph264pay)
    {
    ...
    // Add elements to the pipeline
    gst_bin_add_many(GST_BIN(pipeline), source, capsfilter, videoconvert, x264enc, rtph264pay, NULL);

    // Link the elements
    if (gst_element_link_many(source, capsfilter, videoconvert, x264enc, rtph264pay, NULL) != TRUE)
    {
        ChipLogError(Zcl, "Elements could not be linked.");
        gst_object_unref(pipeline);
        error = CameraError::ERROR_INIT_FAILED;
...
{
    GstElement *pipeline, *source, *capsfilter, *audioconvert, *opusenc, *rtpopuspay;
    ...
    if (!source || !capsfilter || !audioconvert || !opusenc || !rtpopuspay)
    {
        std::cerr << "Failed to create GStreamer elements!" << std::endl;
        if (pipeline)
            gst_object_unref(pipeline);
    ...
    // Link elements to form the pipeline
    gst_bin_add_many(GST_BIN(pipeline), source, capsfilter, audioconvert, opusenc, rtpopuspay, NULL);
    gst_element_link_many(source, capsfilter, audioconvert, opusenc, rtpopuspay, NULL);
...
CameraError CameraDevice::StartVideoStream(uint16_t streamID)
{
    ...
    {
        ChipLogError(Zcl, "Failed to start video pipeline.");
...
CameraError CameraDevice::StartAudioStream(uint16_t streamID)
{
    ...
    if (result == GST_STATE_CHANGE_FAILURE)
    {
        ChipLogError(Zcl, "Failed to start audio pipeline.");
    ...
    if (state != GST_STATE_PLAYING)
    {
        ChipLogError(Zcl, "Audio pipeline did not reach PLAYING state.");
